...
def user_management_id(urlsafe_id):
    ...
    current_user_data = auth0.auth0.users.get(auth0_id,
                                        fields=required_fields,
                                        include_fields=True)
    if "app_metadata" not in current_user_data.keys():
        current_user_data["app_metadata"] = None
    return render_template("admin_user_management_id.html",
                           user_dict=current_user_data)
...
def user_management_delete(urlsafe_id):
    ...
    print(user_email)
    return render_template("admin_user_management_delete.html",
                           user_id=auth0_id,
